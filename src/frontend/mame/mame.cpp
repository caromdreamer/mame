...
void mame_machine_manager::create_custom(running_machine &machine)
{
	...
	if (m_kaileron)
	{
		machine.add_notifier(MACHINE_NOTIFY_FRAME, machine_notify_delegate(&kaileron_adapter::frame_notifier, m_kaileron.get()));
...
void mame_machine_manager::kaileron_frame_done(running_machine &machine, kaileron_frame_mode mode)
{
	if (m_kaileron)
		m_kaileron->frame_done(mode);
}
